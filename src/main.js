...
export default class ObsidianCalendarAgentPlugin extends Plugin {
            ...
            this.addCommand({
                ...
                callback: async () => {
                    try {
                        ...
                        new Notice(`OAuth lỗi: ${error.message}`);
                    }
                }
            });
            this.addCommand({
                ...
                callback: async () => {
                    try {
                        const input = window.prompt("Dán redirect URL hoặc authorization code sau khi login Google:");
                        if (!input?.trim()) {
                        ...
                        new Notice(`OAuth exchange lỗi: ${error.message}`);
                    }
                }
            });
            this.addCommand({
                ...
                callback: async () => {
                    try {
                        ...
                        new Notice(`List events lỗi: ${error.message}`);
                    }
                }
            });
            this.addCommand({
                ...
                callback: async () => {
                    try {
                        ...
                        new Notice(`Gemini test OK. Tool calls: ${result.toolTrace.length}. Xem console để kiểm tra chi tiết.`);
                    }
                    catch (error) {
                        console.error("[obsidian-calendar-agent] gemini hardcoded test failed", error);
                        new Notice(`Gemini test lỗi: ${error.message}`);
                    }
                }
            });
            this.addCommand({
                ...
                callback: async () => {
                    try {
                        ...
                        new Notice(`Undo lỗi: ${error.message}`);
                    }
                }
            });
            this.addCommand({
                ...
                callback: async () => {
                    try {
                        ...
                        new Notice(`Vault context OK. Daily: ${snapshot.dailyNotes.length}, Tasks: ${snapshot.openTasks.length}, Projects: ${snapshot.projects.length}`);
                    }
                    catch (error) {
                        console.error("[obsidian-calendar-agent] vault context test failed", error);
                        new Notice(`Vault context lỗi: ${error.message}`);
                    }
                }
            });
    ...
    async activateView() {
        try {
            ...
            let leaf = workspace.getLeavesOfType(VIEW_TYPE_CALENDAR_AGENT)[0] ?? null;
            if (!leaf) {
